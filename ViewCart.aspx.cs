...
namespace CSharpAssignment
{
    public partial class ViewCart : System.Web.UI.Page
    {
        ...
        void UpdateShoppingCart(string itemID, string itemName, String itemPrice, string unitPrice, String quantity)
        {
            if (Session["ShoppingCart"] == null)
            {
            ...
            {
                DataTable dataTable = (DataTable)Session["ShoppingCart"];
                int index = Convert.ToInt32(GridItemCart.SelectedRow);
        ...
        protected void GridItemCart_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            ...
            string itemName = name.Text;
            string itemquantity = quanti.Text;
            double itemprice = Convert.ToDouble(price.Text) * Convert.ToInt32(itemquantity);
            double unitPrice = Convert.ToDouble(unitPriceEle.Text);
            UpdateShoppingCart(ids, itemName, itemprice.ToString(), unitPrice.ToString(), itemquantity);
            lblTotalAmount.Text = CalculatePrice().ToString();
        }
            ...
            if (CheckUserInfo())
            {
                ...
                for (int i = 0; i < GridItemCart.Rows.Count; i++)
                {
                    ...
                    else
                    {
                        ...
                        Literal1.Text = Literal1.Text + "Product " + productName + " only have " + currentQuantity + " left. ";
                    }
                }
                if (checkBooking)
                {
